...
## 目的
    ...
    前端学习
    前端从基础开始包括html、css、js，而且这些都跟编程界的其他语言一样，都在随着时间的流逝，版本更迭，功能增加。但是为什么前端在整个体系架构中
    地位不高，或者说随着框架的不断推出、功能增加，需要不断的学习，但是得到的回报并不成正比。
    原因如下：
    1、前端一般都是负责呈现的，与用户打交道，实际的业务需求交给了后端。复杂度上不来
    2、前端的调试工具也多，同时前端不像后端语言，运行在服务器，且多是编译后的机器语言。前端语言会随着页面，也就是http文件一起以明文的形式出现在所有人面前。
    3、1说明了复杂度不高，2说明了源码获取难度不高。且伴随搜索、AI、大数据等技术的不断发展，它们在处理这些复杂度不高的事情上有天然优势。造成前端扁平化的现状。
    但是前端也有自己在整个全栈解决方案中不可替代的作用，就是越来越多的用户交互方案都依靠前端。

## 前端学习路线

    从基础的html、css、js学习，认识前端，然后锚定一个较为流行的框架，从小型项目开始，慢慢的体会，慢慢的做大做强。
